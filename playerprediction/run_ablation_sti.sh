...
ABLATION_ROOT="${PROJECT_ROOT}/playerprediction"
...
GRAPH_LAYERS="${GRAPH_LAYERS:-0}"
GRAPH_MODE="${GRAPH_MODE:-standard}"
...
MODEL_NAME="${MODEL_NAME:-playerpred-${TASK,,}-${DATASET}-vf${VISUAL_FILM_LAYERS}-ff${FUSION_FILM_LAYERS}-${FUSION_MODE}-${GRAPH_MODE}-${CLASSIFIER_MODE}-gl${GRAPH_LAYERS}-seed${SEED}-cl${CENTER_LOSS_WEIGHT}-sc${SUPCON_WEIGHT}-j${JEPA_WEIGHT}-jm${JEPA_MASK_COUNT}}"
...
python train.py \
  ...
  --jepa_mask_count "${JEPA_MASK_COUNT}" \
  --jepa_momentum "${JEPA_MOMENTUM}" \
  "${WANDB_ARGS[@]}"
