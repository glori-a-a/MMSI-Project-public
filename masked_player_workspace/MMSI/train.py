...
def parse_args():
    ...
    parser.add_argument('--graph_mode', type=str, default='standard', choices=['standard', 'speaker_centered', 'speaker_contextual'],
                        help='Graph interaction mode for player relation modeling')
    parser.add_argument('--classifier_mode', type=str, default='standard', choices=['standard', 'relational'],
    ...
    parser.add_argument('--jepa_weight', type=float, default=0.0, help='Weight for JEPA-style masked player prediction loss')
    parser.add_argument('--jepa_mask_count', type=int, default=1, help='Number of non-speaker player nodes to mask per sample for JEPA loss')
    parser.add_argument('--jepa_momentum', type=float, default=0.99, help='EMA momentum for JEPA target projector')
    ...
    for language_tokens, mask_idxs, keypoint_seqs, speaker_labels, task_labels in dataloader:
        ...
        with torch.cuda.amp.autocast():
            model_outputs = model(
                ...
                return_aux=(args.jepa_weight > 0), compute_jepa=(args.jepa_weight > 0),
                jepa_mask_count=args.jepa_mask_count,
            )
            if args.jepa_weight > 0:
                outputs, features, projections_view1, aux_outputs = model_outputs
                jepa_term = aux_outputs.get('jepa_loss', outputs.new_tensor(0.0))
            else:
                outputs, features, projections_view1 = model_outputs
                jepa_term = outputs.new_tensor(0.0)
            ...
            loss = ce_loss + args.center_loss_weight * center_term + args.supcon_weight * supcon_term + args.jepa_weight * jepa_term
        ...
        scaler.update()
        if args.jepa_weight > 0:
            model.update_jepa_target(args.jepa_momentum)
        ...
        jepa_loss_meter.update(jepa_term.item(), task_labels.size(0))
        progbar.add(args.batch_size, values=[('loss', loss.item())])

    return train_loss.avg, ce_loss_meter.avg, center_loss_meter.avg, supcon_loss_meter.avg, jepa_loss_meter.avg
...
def main():
    ...
    if args.jepa_weight < 0.0:
        raise ValueError("--jepa_weight must be >= 0.")
    if args.jepa_mask_count < 1:
        raise ValueError("--jepa_mask_count must be >= 1.")
    if not 0.0 < args.jepa_momentum < 1.0:
    ...
    for epoch in range(args.epochs):
        train_loss, ce_loss, center_term, supcon_term, jepa_term = train_one_epoch(
        ...
        print(f"JEPA Loss: {jepa_term:.3f}")
