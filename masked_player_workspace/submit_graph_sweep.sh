...
for graph_layers in ${GRAPH_LAYER_OPTIONS}; do
  model_name="ablation_${TASK,,}_${DATASET}_vf${VISUAL_FILM_LAYERS}_ff${FUSION_FILM_LAYERS}_${FUSION_MODE}_${GRAPH_MODE}_gl${graph_layers}_cl${CENTER_LOSS_WEIGHT}_sc${SUPCON_WEIGHT}"
  echo "Submitting ${model_name}"
  sbatch \
    --job-name="${model_name}" \
    --export=ALL,DATASET="${DATASET}",TASK="${TASK}",USE_WANDB=1,WANDB_PROJECT="${WANDB_PROJECT}",WANDB_ENTITY="${WANDB_ENTITY}",VISUAL_FILM_LAYERS="${VISUAL_FILM_LAYERS}",FUSION_FILM_LAYERS="${FUSION_FILM_LAYERS}",FUSION_MODE="${FUSION_MODE}",GRAPH_MODE="${GRAPH_MODE}",GRAPH_LAYERS="${graph_layers}",CENTER_LOSS_WEIGHT="${CENTER_LOSS_WEIGHT}",SUPCON_WEIGHT="${SUPCON_WEIGHT}",MODEL_NAME="${model_name}",WANDB_RUN_NAME="${model_name}" \
