...
WANDB_PROJECT="${WANDB_PROJECT:-mmsi-bart-cached-vit-large}"
WANDB_ENTITY="${WANDB_ENTITY:-}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-bart_sti_keypoint_cached_vit_youtube_large}"

DATA_ROOT="${PROJECT_ROOT}/data/mmsi"
BENCHMARK_ROOT="${DATA_ROOT}/benchmark"
KEYPOINT_ROOT="${DATA_ROOT}/keypoints"
VIT_CACHE_DIR="${PROJECT_ROOT}/data/vit_features/youtube_large"
...
python train.py \
  --model_name "bart_sti_keypoint_cached_vit_youtube_large" \
  --task "STI" \
  ...
  --checkpoint_save_dir "${PROJECT_ROOT}/checkpoints/bart_sti_keypoint_cached_vit_youtube_large" \
  --language_model "bart" \
  --text_pooling "mask" \
